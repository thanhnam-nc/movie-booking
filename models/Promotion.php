...
class Promotion {
    ...
    public function getActivePromotions() {
        $sql = "SELECT promotion_id, promo_code, discount_type, discount_value, description, min_tickets, min_amount, applicable_seat_types FROM $this->table_name WHERE start_date <= NOW() AND end_date >= NOW() ORDER BY start_date ASC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
