...
class User {
    ...
    public function registerOAuth($provider, $oauth_id) {
        $sql = "INSERT INTO $this->table_name 
        SET full_name=:full_name, email=:email, password=:password, phone=:phone, birthday=:birthday, address=:address, role=:role, status=:status, oauth_provider=:oauth_provider, oauth_id=:oauth_id";
        $stmt = $this->conn->prepare($sql);
